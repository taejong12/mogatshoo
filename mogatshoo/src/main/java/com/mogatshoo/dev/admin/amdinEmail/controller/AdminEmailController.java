...
public class AdminEmailController {
	...
	public String emailSendPage(@PathVariable("serialNumber") String serialNumber, Model model,
		...
		try {
			...
			// 이메일 제목 미리보기 생성
			String emailSubject = String.format("%s님 득표율 %.2f%%를 넘겼습니다. 축하드립니다!", questionStats.getTopVotedName(),
					questionStats.getVotingRate());
	 ...
	 */
	@PostMapping("/send")
	public String sendEmail(@RequestParam("serialNumber") String serialNumber,
		...
		try {
			...
			// 이메일 전송
			AdminEmailEntity emailResult = adminEmailService.sendWinnerEmailFromStatus(questionStats, senderId,
					customContent, attachmentFile);

			if (emailResult != null && "SENT".equals(emailResult.getEmailStatus())) {
				String successMessage = String.format("%s님에게 당첨 이메일이 성공적으로 전송되었습니다.", questionStats.getTopVotedName());
	...
	@ResponseBody
	public ResponseEntity<Map<String, Object>> sendEmailAjax(@RequestParam("serialNumber") String serialNumber,
			@RequestParam(value = "customContent", required = false) String customContent,
			@RequestParam(value = "attachmentFile", required = false) MultipartFile attachmentFile) {
	...
	/**
	 * 이메일 전송 이력 조회 - @PathVariable 추가
	 */
	@GetMapping("/history/{serialNumber}")
	public String emailHistory(@PathVariable("serialNumber") String serialNumber, Model model) {
		try {
	...
	/**
	 * 이메일 재전송 - @PathVariable 추가
	 */
	@PostMapping("/resend/{emailId}")
	public String resendEmail(@PathVariable("emailId") Long emailId, RedirectAttributes redirectAttributes) {
		try {
			Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
			String senderId = authentication.getName();
